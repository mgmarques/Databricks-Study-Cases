...
def customer_claim_policy():
    ...
    policy = dlt.readStream(f"{catalog}.{silver_schema}.policy").drop("ingested_at", "source_file")
    # Read the cleaned claim records
    claim = dlt.readStream(f"{catalog}.{silver_schema}.claim")
    # Read the cleaned customer records
    customer = dlt.readStream(f"{catalog}.{silver_schema}.customer").drop("ingested_at", "source_file")
